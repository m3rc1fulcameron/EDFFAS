	...
	class inputValidate {
		...
		private function stringLen($rule) {
			...
			if (isset($rule["methodArgs"]["len"]["min"]) && $rule["methodArgs"]["len"]["min"] !== -1) {
				if (strlen($string) <= $rule["methodArgs"]["minLen"]) {
					user_error((isset($rule["methodArgs"]["len"]["failMsg"]) ? $rule["methodArgs"]["len"]["failMsg"]:"Validation error: supplied string is too short."),E_USER_NOTICE);
				}
			}
			
			if (isset($rule["methodArgs"]["len"]["max"]) && $rule["methodArgs"]["len"]["max"] !== -1) {
				if (strlen($string) >= $rule["methodArgs"]["minLen"]) {
					user_error((isset($rule["methodArgs"]["len"]["failMsg"]) ? $rule["methodArgs"]["len"]["failMsg"]:"Validation error: supplied string is too long."),E_USER_NOTICE);
				}
			}
		}
		...
		private function matches($rule) {
			if (!isset($rule["methodArgs"]["strings"]) || count($rule["methodArgs"]["strings"]) <2) {
				user_error("Invalid rule structure: must supply at least two strings to compare.",E_USER_NOTICE);
				return false;
			}
			...
			foreach($strings as $string) {
				if (!isset($this->input[$string])) {
					user_error("Invalid rule structure: input '" . $string . "' does not exist in input table.",E_USER_NOTICE);
					return false;
				}
			}
			...
			if (isset($rule["methodArgs"]["ci"]) && $rule["methodArgs"]["ci"]) {
				...
				for ($i=1;$i<count($strings);$i++) {
					if (strtolower($this->input[$strings[$i]]) != $base) {
						user_error("Validation error: string '" . $strings[$i] . "' (" . $this->input[$strings[$i]] . ") does not match string '" . $strings[0] . "' (" . $base . ").");
					}
				}
			} else {
		...
		private function existsInDatabaseTable($rule) {
			...
			if (!isset($rule["methodArgs"]["table"]) || !isset($rule["methodArgs"]["column"]) || !isset($rule["methodArgs"]["search"])) {
			...
			$queryHandle->bindParam(":search",$rule["methodArgs"]["search"]);
			$queryHandle->execute();
			
			$result = $queryHandle->fetchAll(PDO::FETCH_NUM);
			
			if (count($result) == 0) {
				user_error("Validation error: no entry in table '" . $rule["methodArgs"]["table"] . "' with value '" . $rule["methodArgs"]["search"] . "' in column '" . $rule["methodArgs"]["column"] . "' found in database.");
			}
		}
		...
		private function username($rule) {
			...
			$this->existsInDatabaseTable(array(
				"inputName" => $rule["inputName"],
				"methodArgs" => $rule["methodArgs"]
			));
		}
		...
		private function email($rule) {
			...
			if (!filter_var($email,FILTER_VALIDATE_EMAIL)) {
				user_error($failMsg,E_USER_NOTICE);
			}
		}
				...
				switch ($rule["methodArgs"]["securityLevel"]) {
					...
					case 2: //Length greater than 8 w/ letter & number
						...
						if (!preg_match("/^(?=.*[a-zA-Z])(?=.*\d).*$/i",$password)) {
							user_error((isset($rule["methodArgs"]["regex"]["failMsg"]) ? $rule["methodArgs"]["regex"]["failMsg"]:"Validation error: password must contain at least one of each of the following: a letter and a number."),E_USER_NOTICE);
						}
					...
					case 3: //Length greater than 8 w/ UC and LC letter + number
						...
						if (!preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).*$/i",$password)) {
							user_error((isset($rule["methodArgs"]["regex"]["failMsg"]) ? $rule["methodArgs"]["regex"]["failMsg"]:"Validation error: password must contain at least one of each of the following: an upper-case letter, a lower-case letter, and a number."),E_USER_NOTICE);
						}
					...
					case 4: //Length greater than 8 w/ UC + LC letter + number + symbol
						...
						if (!preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$-/:-?{-~!\"^_`\[\]]).*$/i",$password)) {
							user_error((isset($rule["methodArgs"]["regex"]["failMsg"]) ? $rule["methodArgs"]["regex"]["failMsg"]:"Validation error: password must contain at least one of each of the following: an upper-case letter, a lower-case letter, a number, and a symbol."),E_USER_NOTICE);
						}
					...
					default:
						...
						if (isset($rule["methodArgs"]["regex"]["pattern"])) {
							if (!preg_match($rule["methodArgs"]["regex"]["pattern"],$password)) {
								user_error((isset($rule["methodArgs"]["regex"]["failMsg"]) ? $rule["methodArgs"]["regex"]["failMsg"]:"Validation error: string does not match the supplied regular expression."),E_USER_NOTICE);
							}
						}
				}
			}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	$input = array();
	$rules = array(
		array("inputName" => "un", "method" => "username", "methodArgs" => array("table" => "testTable", "column" => "testColumn", "search" => "testSearch")),
		array("inputName" => "em", "method" => "email"),
		array("inputName" => "pw", "method" => "password", "methodArgs" => array("securityLevel" => 2)),
